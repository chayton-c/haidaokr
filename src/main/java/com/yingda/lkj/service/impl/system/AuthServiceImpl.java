...
public class AuthServiceImpl implements AuthService {
    ...
    public void updateAuthBackstage(String roleId, List<String> menuIds) {
        init();
        List<String> templateMenuIds = menuService.showDown().stream().filter(x -> x.getType() == Menu.BACK_STAGE).map(Menu::getId).collect(Collectors.toList());
    ...
    public List<Menu> getValuableMenus(User user) {
        ...
        return menuService
                .showDown()
                .stream()
                .filter(x -> x.getType() == Menu.BACK_STAGE)
