...
public class RequestUtil {
    ...
    public static void checkParameters(HttpServletRequest req, Map<String, String> attributes) throws CustomException {
        for (Map.Entry<String, String> entry : attributes.entrySet()) {
            String paramName = entry.getKey();
            if (StringUtils.isEmpty(req.getParameter(paramName)))
                throw new CustomException(JsonMessage.PARAM_INVALID, String.format("%s不能为空", entry.getValue()));
        }
    }
