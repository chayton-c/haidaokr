...
public class ExcelUtil {
    ...
    public static List<ExcelSheetInfo> readExcelFile(MultipartFile excelFile) throws IOException {
        ...
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            ...
            Map<Integer, ExcelRowInfo> rowInfoMap = excelRowInfos.stream().collect(Collectors.toMap(ExcelRowInfo::getLineIndex, x -> x));
    ...
    private static List<ExcelRowInfo> getExcelRowInfo(Sheet sheet) {
        ...
        for (int lineIndex = 0; lineIndex <= lastRowNum; lineIndex++) {
            // 获得当前行
            Row row = sheet.getRow(lineIndex);
            if (row == null) {
                excelRowInfos.add(new ExcelRowInfo(lineIndex, Collections.singletonList("")));
                continue;
            }
            ...
            excelRowInfos.add(new ExcelRowInfo(lineIndex, Arrays.asList(cells)));
        }
