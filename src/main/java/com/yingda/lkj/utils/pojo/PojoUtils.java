...
public class PojoUtils {
     ...
     */
    public static <K, V> List<V> copyPojoList(Iterable<K> kList, Class<V> classV) throws ReflectiveOperationException {
    ...
    public static <T> Map<String, Object> toMap(Class<T> tClass, T t) {
        ...
        for (Field declaredField : declaredFields) {
            ...
            if (declaredField.isSynthetic())
                continue;
            if (declaredField.getName().equals("LOGGER"))
                continue;
            declaredField.setAccessible(true);

            try {
                result.put(declaredField.getName(), declaredField.get(t));
            } catch (Exception e) {
                LoggerFactory.getLogger(tClass).error("枚举类遍历异常，不可能的");
            }
        }
