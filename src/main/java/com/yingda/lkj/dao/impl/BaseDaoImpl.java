...
import com.yingda.lkj.beans.pojo.system.HibernateCustomResultTransformer;
...
//        NativeQuery<T> q = this.getCurrentSession().createNativeQuery(sql).unwrap(NativeQueryImpl.class).setResultTransformer(Transformers.aliasToBean(clazz));
        NativeQuery<T> q = this.getCurrentSession().createNativeQuery(sql).unwrap(NativeQueryImpl.class).setResultTransformer(new HibernateCustomResultTransformer(clazz));
        setParam(q, params);
        return q.list();
    }
