...
import com.yingda.lkj.beans.pojo.organization.OrganizationSelectTreeNode;
...
public class OrganizationController extends BaseController {
    ...
    public Json getSubordinateOrganization() {
        ...
        List<Organization> organizations = organizationClientService.getSlaves(checkedOrganizationId).stream()
        ...
        organizationStream = organizationStream.skip((long) (currentPage - 1) * pageSize).limit(pageSize);
    ...
    public Json delete() throws Exception {
        ...
        for (String organizationId : organizationIdArr) {
            List<Organization> slave = organizationClientService.getSlaves(organizationId);
            if (!slave.isEmpty())
    ...
    @RequestMapping("/organizationSelectTreeNode")
    @ResponseBody()
    public Json getOrganizationSelectTreeNode() {
        List<Organization> bureaus = organizationClientService.getBureaus();
        List<OrganizationSelectTreeNode> organizationSelectTreeNodes = bureaus.stream().map(OrganizationSelectTreeNode::new).collect(Collectors.toList());

        return new Json(JsonMessage.SUCCESS, Map.of("organizationSelectTreeNodes", organizationSelectTreeNodes));
    }

    @RequestMapping("/initOrganizationCasecade")
    @ResponseBody()
    public Json initOrganizationCasecade() throws CustomException {
        Map<String, Object> attributes = new HashMap<>();

        if (Role.ADMIN.equals(getUser().getUserName())) {
            List<Organization> completeTree = organizationService.getCompleteTree();
            List<OrganizationSelectTreeNode> organizationSelectTreeNodes = completeTree.stream().map(OrganizationSelectTreeNode::new).collect(Collectors.toList());
            attributes.put("organizationSelectTreeNodes", organizationSelectTreeNodes);
            attributes.put("organizations", completeTree);
            return new Json(JsonMessage.SUCCESS, attributes);
        }

        // 用户最低级的组织id(如果用户等级是段长，userLowestLevelOrganizationId就是所在段id)
        String userLowestLevelOrganizationId = getUserLowestLevelOrganizationId();

        Organization organization = organizationClientService.getById(userLowestLevelOrganizationId);
        List<Organization> slaves = organizationClientService.getSlaves(userLowestLevelOrganizationId);
        organization.setOrganizationList(slaves);
        Organization parentTree = organizationClientService.getParentTree(organization);

        List<OrganizationSelectTreeNode> organizationSelectTreeNodes = List.of(parentTree).stream().map(OrganizationSelectTreeNode::new).collect(Collectors.toList());

        attributes.put("organizationSelectTreeNodes", organizationSelectTreeNodes);
        attributes.put("organizations", List.of(parentTree));
